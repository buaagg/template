...
namespace SAM {
	...
	void init() {
		if (cnt) rep(i, cnt) pool[i] = Node();
		cnt = 1; root = &pool[0]; last = root;
	}
...
int n, m; char S[N], buf[N];
 
void calc() {
	int ans = 0; vector <int> vec;
	for (int i = 0; i < SAM::cnt; i++) {
		vec.push_back(i); pool[i].way = pool[i].sum = 0;
	}
	sort(vec.begin(), vec.end(), cmp);
	root->way = 1; root->sum = 0;
	foreach (it, vec) {
		int i = *it;
		Node *p = &pool[i];
...
int main(){
	while (scanf("%d", &n) != EOF) {
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = 0; j < len; j++) SAM::add(buf[j] - '0');
			SAM::add(10);
		}
		calc();
	}
